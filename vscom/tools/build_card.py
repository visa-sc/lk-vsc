...
(function () {
  ...
  var qs = new URLSearchParams(location.search), utm = {}, hasTag = false;
  ["utm_source","utm_medium","utm_campaign","utm_content","utm_term","gclid","gbraid","wbraid",
   "gad_source","gad_campaignid","yclid","fbclid"].forEach(function (k) {
    if (qs.get(k)) { utm[k] = qs.get(k).slice(0, 200); hasTag = true; }
  });

  // Метки перехода помним 90 дней: человек пришёл с рекламы, ушёл и вернулся
  // напрямую — его переход в кабинет партнёра всё равно засчитается рекламе
  // (поле touch в журнале: первый и последний заход).
  var touch = {};
  try {
    touch = JSON.parse(localStorage.getItem("vsc_ads") || "null") || {};
    var rh = ""; try { rh = document.referrer ? new URL(document.referrer).hostname : ""; } catch (e) {}
    var ext = rh && !/(^|\\.)visa-sc\\.com$/.test(rh);
    if (hasTag || ext) {
      var t = { ref: ext ? document.referrer.slice(0, 300) : "",
                landing: (location.pathname + location.search).slice(0, 300), at: Date.now() };
      for (var k in utm) t[k] = utm[k];
      if (!touch.first) touch.first = t;
      touch.last = t;
      localStorage.setItem("vsc_ads", JSON.stringify(touch));
    }
  } catch (e) {}
  ...
  function track(kind, extra) {
    var body = JSON.stringify({
      ...
      page: location.href, referrer: document.referrer || "", utm: utm, touch: touch
    });
    try {
    ...
    if (extra && typeof window.gtag === "function") gtag("event", extra.event, extra.params);
  }

  // Просмотр страницы — раз за сессию. Серверы Google, открывающие ссылку
  // объявления, скрипты не выполняют, так что в журнал попадают живые люди.
  try {
    if (!sessionStorage.getItem("vsc_view")) { sessionStorage.setItem("vsc_view", "1"); track("view"); }
  } catch (e) { track("view"); }
