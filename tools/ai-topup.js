...
for (const x of s.svc || []) console.log("  " + (x.title + ":").padEnd(32) + money(x.usd) + " за " + x.calls + " обращений");
if (s.undividedUsd > 0.005) console.log("  " + "без разделения (до 16.09):".padEnd(32) + money(s.undividedUsd));
if (s.scanner.usd > 0.005) console.log("  " + "сканер паспортов:".padEnd(32) + money(s.scanner.usd) + " за " + s.scanner.docs + " документов");
