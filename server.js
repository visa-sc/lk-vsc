...
const KB_DOC_GROUPS = [
  { stage: "Начало оформления (этап 0)", note: "Шенген: оба паспорта. Япония (корр. Насти 03.07): только внутренний — загран перенесён на «Первичный сбор».", docs: [
    { n: "Внутренний паспорт (1-й разворот, прописка, последний разворот)", c: "всегда (обе визы)", r: true },
    { n: "Загран. паспорт (в который запрашиваем визу)", c: "только Шенген (у Японии — на этапе 1)", r: true }
  ]},
  ...
  { stage: "Первичный сбор документов (этап 1) — Япония", note: "Карта Насти 03.07: обязательные + документы по роду занятий (на этом этапе НЕобязательные — станут обязательными на «Подготовке документов»).", docs: [
    { n: "Загран. паспорт (в который запрашиваем визу)", c: "всегда (перенесён с этапа 0)", r: true },
  ...
  { stage: "Подготовка документов (этап 2) — Япония", note: "НОВОЕ 03.07 (корр. Насти): у Японии появилась основная область — все опциональные документы этапа 1 становятся ОБЯЗАТЕЛЬНЫМИ (кроме 3-НДФЛ). Документы, не положенные по цели поездки (напр. приглашение при туризме), не запрашиваются. Плюс, как раньше: подобласть «перед подачей» (за 7 дней до даты записи) и свободная зона загрузки.", docs: [
    { n: "Все обязательные этапа 1 (загран, приглашение+план, авиабилеты, паспорт спонсора, св-во о рождении, св-во о браке)", c: "по своим условиям", r: true },
...
const LK_TESTER_SPEC = {
  ...
  stageSees: {
    0: [
      ...
      "Загрузка паспортов на каждого заявителя: Шенген — два (внутренний + загран); Япония — только внутренний (загран на «Первичном сборе», корр. Насти 03.07). До заполнения опросника (направление неизвестно) показываются оба.",
      "Кнопка «Заполнить опросник»; паспорта можно загрузить и до опросника — тогда спросим ФИО.",
      "Опросник Шенген включает новые вопросы (корректировки Риты): «Город подачи» + «Прописка в паспорте РФ в городе подачи?» (при «Нет» — город прописки) и блок «Данные принимающей стороны» при цели «Частный визит». Детали — во вкладке «Опросники».",
      "Над списком — красная плашка «Необходимо загрузить все документы ниже»; зеленеет, когда оба паспорта прикреплены."
    ],
    1: [
      "Паспорта этапа 0 (кумулятивно, если ещё не загружены) + первый набор условных документов по ответам опросника. Япония: здесь же обязательный загранпаспорт (перенесён с этапа 0) + НЕобязательные документы по роду занятий и банковская справка (станут обязательными на этапе 2) — корр. Насти 03.07.",
  ...
  contentious: [
    ...
    "✓ РЕШЕНО 03.07 (корр. Насти П.): у Японии на «Подготовке документов» появилась основная область — все опциональные документы «Первичного сбора» становятся обязательными (кроме 3-НДФЛ); также загран перенесён с этапа 0 на «Первичный сбор», добавлены «Свидетельство о браке» (если супруг едет) и «Документ, подтверждающий родство со спонсором» (спонсор-родственник, этап 2).",
  ...
  directions: {
    ...
    japan: {
      ...
      docsByStage: {
        "0": [
          { label: "Внутренний паспорт (1-й разворот, прописка, последний разворот)", when: "всегда (загран перенесён на «Первичный сбор» — корр. Насти 03.07)" }
        ],
        "1": [
          { label: "Загран. паспорт (в который запрашиваем визу)", when: "всегда (обяз.)" },
      ...
      qrecs: [],
      docRecs: [
        "✓ 03.07 (корр. Насти П.): на «Подготовке документов» у Японии появилась ОСНОВНАЯ область документов — кумулятивный список, где все опциональные документы «Первичного сбора» становятся обязательными (кроме «Декларации 3-НДФЛ»). Также загранпаспорт перенесён с «Начала оформления» на «Первичный сбор» (на этапе 0 — только внутренний).",
  ...
  uploadAreas: [
    {
      ...
      composition: [
        "Внутренний паспорт (1-й разворот, прописка, последний разворот) — обеим визам, обязателен.",
        "Загранпаспорт, в который запрашиваем визу — только Шенген (у Японии с 03.07 загран запрашивается на «Первичном сборе»).",
        "До заполнения опросника (pre-applicant) показываются оба паспорта — направление ещё неизвестно."
      ],
      color: [
    ...
    },
    {
      ...
      composition: [
        ...
        "Япония (карта Насти 03.07) — обязательность ЭТАПНАЯ, не статусная. Обязательные: «Загран. паспорт» (перенесён с этапа 0); «Приглашение» + «План поездки» (цель ≠ Туризм); «Авиабилеты» (есть свои); «Внутр. паспорт спонсора»; «Свидетельство о рождении» (<18); «Свидетельство о браке» (в браке + супруг едет).",
    ...
    },
    {
      ...
      appears: [
        ...
        "Япония, кумулятивно: незагруженный внутренний паспорт + весь список этапа 1, где все опциональные становятся ОБЯЗАТЕЛЬНЫМИ (кроме «Декларации 3-НДФЛ»), + «Документ, подтверждающий родство со спонсором» (спонсор-родственник). Свободная зона и подобласть «перед подачей» сохраняются."
      ],
      composition: [
  ...
  screens: [
    { stage: 0, title: "0 · Начало оформления", sees: [
      ...
      "Опросник Шенген: новые вопросы (корректировки Риты) — «Город подачи» + «Прописка в паспорте РФ в городе подачи?» (при «Нет» — город прописки) и блок «Данные принимающей стороны» при цели «Частный визит» (см. «Опросники»).",
      "Над списком — красная плашка «Необходимо загрузить все документы ниже»; зеленеет, когда оба паспорта прикреплены."
    ], dot: [
...
function buildUploadBlocksForApplicantStats(state, lead, stageIndex) {
  ...
  if (isJp) {
    // ── Япония (карта Насти П., корр. 03.07.2026) ──
    // Этап 0: только внутренний паспорт (загран перенесён на «Первичный сбор»).
    // Этап 1: обязательные — загран, приглашение+план (цель ≠ Туризм), паспорт
    ...
    const jpStage0 = [stage0[0]]; // только внутренний паспорт
    ...
    const jpConditional = (stage2Mode) => {
      const list = [];
      list.push({ field: "mainPassport", label: "Загран. паспорт (в который запрашиваем визу)", optional: false });
