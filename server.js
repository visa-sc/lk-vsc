...
const LK_TESTER_SPEC = {
  ...
  stageSees: {
    0: [
      "Карточка «Быстрая анкета для менеджера»: ФИО + страна поездки + даты поездки + «Отправить» (новое, каркас).",
  ...
  actions: [
    ...
    { sys: "ЛК", name: "Выдача сессии-cookie при входе (Фаза 1 безопасности)", when: "Любой успешный вход: SMS-код, Face ID / Touch ID по номеру, Face ID кнопкой", details: [
      "Клиенту ставится подписанная cookie-сессия voyo_sess (HMAC-SHA256, срок 90 дней).",
      "Добавлен служебный эндпоинт /api/me (кто сейчас в сессии).",
      "Для клиента ничего не меняется — обратная совместимость 100% (данные пока принимаются и по номеру в ссылке).",
      "Фаза 2 (закрыть доступ по номеру в ссылке) — запланирована отдельно."
    ] },
    ...
    { sys: "amoCRM", name: "Задача по «Быстрой анкете для менеджера» (новое, каркас)", when: "Клиент отправил быструю анкету (ФИО + страна поездки + даты поездки) на этапе «Начало оформления»", details: [
      "Создаётся задача на текущего ответственного по сделке (срок — сразу) с текстом анкеты.",
      "Данные сохраняются для будущей авто-подстановки в опросник — сама подстановка пока НЕ реализована (в планах).",
  ...
  screens: [
    { stage: 0, title: "0 · Начало оформления", sees: [
      "Карточка «Быстрая анкета для менеджера»: ФИО + страна поездки + даты поездки + «Отправить» → задача в amoCRM на ответственного (новое, каркас; авто-подстановка в опросник — в планах).",
