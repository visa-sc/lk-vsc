...
// Собирает prefill-поля из сохранённой быстрой анкеты для leadId (пустой объект,
// если анкеты нет). isJp — японский опросник (другие имена полей дат, нет страны).
function opquickPrefill(leadId, isJp) {
  const oq = (loadOpQuick() || {})[String(leadId || "")];
  const p = {};
  if (!oq) return p;
  if (oq.fio) p.fullName = String(oq.fio).trim().slice(0, 200);
  if (!isJp && oq.country) { const c = opquickMatchCountry(oq.country); if (c) p.visaCountry = c; }
  if (oq.dates) {
...
<!-- s49 СКРЫТО 16.06 (карточка убрана у клиента; вернуть = раскомментировать <li> ниже):
<li><b>«Быстрая анкета для менеджера» (новое):</b> на этапе «Начало оформления» — карточка с полями ФИО + страна поездки + даты поездки и кнопкой «Отправить». По отправке создаётся задача в amoCRM на текущего ответственного по сделке (срок — сразу), а введённые данные <b>автоматически подставляются в опросник первого заявителя</b>: ФИО — всегда; страна — если распознана из списка стран; даты — если распознан формат (ДД.ММ.ГГГГ / ГГГГ-ММ-ДД), иначе клиент заполнит сам. Финальный список полей анкеты уточняется (Ксюша/РОП).</li>
-->
</ul>`;
...
app.get("/admin/kb/documents", requireStaff, (req, res) => {
  try {
    ...
    b += `<div class="card muted"><b>Когда документ становится обязательным (логика Насти, июнь 2026).</b> Бейдж «по статусу» означает: документ открыт для загрузки заранее, а обязательность зависит от <b>статуса сделки в amoCRM + «Даты записи на подачу»</b>:
<ul>
...
const LK_TESTER_SPEC = {
  ...
  stageSees: {
    0: [
      // s49 СКРЫТО 16.06 (карточка убрана у клиента; вернуть = раскомментировать строку ниже):
      // "Карточка «Быстрая анкета для менеджера»: ФИО + страна поездки + даты поездки + «Отправить»; введённые данные автоподставляются в опросник 1-го заявителя (новое).",
      "Загрузка двух паспортов (внутренний + загран) на каждого заявителя.",
      "Кнопка «Заполните опросник»; паспорта можно загрузить и до опросника — тогда спросим ФИО.",
      "Над списком — красная плашка «Необходимо загрузить все документы ниже»; зеленеет, когда оба паспорта прикреплены."
    ],
    1: [
      "Паспорта этапа 0 (кумулятивно, если ещё не загружены) + первый набор условных документов по ответам опросника.",
      "Обязательность — по статусу сделки + «Дате записи на подачу» (см. блок «Когда документ обязателен» ниже).",
  ...
  actions: [
    ...
    // s49 СКРЫТО 16.06 (карточка убрана у клиента; вернуть = раскомментировать блок ниже):
    // { sys: "amoCRM", name: "Задача по «Быстрой анкете для менеджера» (новое)", when: "Клиент отправил быструю анкету (ФИО + страна поездки + даты поездки) на этапе «Начало оформления»", details: [
    //   "Создаётся задача на текущего ответственного по сделке (срок — сразу) с текстом анкеты.",
    //   "Введённые данные автоматически подставляются в опросник первого заявителя: ФИО — всегда; страна — если распознана из списка; даты — если распознан формат (иначе клиент заполнит сам).",
    //   "Финальный список полей анкеты уточняет Ксюша / РОП.",
  ...
  uploadAreas: [
    {
      ...
      composition: [
        ...
        "Оба паспорта обязательны всегда."
      ],
      color: [
    ...
    },
    {
      ...
      composition: [
        "Шенген: «Электронное фото» (если «Страна оформления/услуга» в CRM = Испания/Португалия/Кипр); «Приглашение» (если цель ≠ Туризм).",
        "Япония: «Приглашение» + «План поездки» (цель ≠ Туризм); «Авиабилеты» (есть свои); «Внутр. паспорт спонсора»; «Свидетельство о рождении» (<18); «Справка с работы» / «Справка с учёбы» (по роду занятий).",
        "Обязательность — по статусу сделки + «Дате записи на подачу»: ранние статусы — необязательно; «Запись сделана / Электронное рассмотрение / Оформлен выкуп» — обязательно ЯДРО оформления (справка с работы/учёбы, паспорт спонсора, своё проживание, авиабилеты, 2-й загран, билеты в 3-ю страну, свид-во о рождении) за ≤7 рабочих дней; финальные статусы или <3 кал. дней — всё."
      ],
    ...
    },
    {
      ...
      appears: [
        "Всегда на этапе 2 у Шенгена.",
        "Кумулятивно: всё незагруженное с этапов 0–1 + до 12 условных документов (фото действующей/последней визы, спонсорские, свид-во о рождении, 2-й загран, проживание, транспорт, билеты в 3-ю страну, ВНЖ, справка с работы/учёбы, страховка, посадочные талоны)."
      ],
      composition: [
      ...
      notes: ["У Японии на этапе 2 основной области нет — вместо неё свободная зона и подобласть «перед подачей» (если задана дата записи)."]
    },
    {
    ...
    },
    {
      ...
      composition: [
        "На «Ожидании подачи» — блок «Ваши готовые документы» (скачивание файлов, которые положил отдел оформления).",
  ...
  screens: [
    { stage: 0, title: "0 · Начало оформления", sees: [
      // s49 СКРЫТО 16.06 (карточка убрана у клиента; вернуть = раскомментировать строку ниже):
      // "Карточка «Быстрая анкета для менеджера»: ФИО + страна поездки + даты поездки + «Отправить» → задача в amoCRM на ответственного; введённые данные автоподставляются в опросник 1-го заявителя (новое).",
      "Загрузка двух паспортов (внутренний + загран) на каждого заявителя; кнопка «Заполните опросник».",
...
app.get("/questionnaire", async (req, res) => {
  try {
    ...
    // Автоподстановка из «Быстрой анкеты для менеджера» (s49): только первичное
    // заполнение 1-го заявителя (не edit) — переносим ФИО/страну/даты в ПУСТЫЕ поля.
    // Кривые значения опросник не сломают: select добавит опцию, date-инпут отбросит
    // невалидную дату; редактирование (isEdit) грузит реальные ответы и сюда не попадает.
    if (!isEdit && applicantIndex === 1) {
      try {
