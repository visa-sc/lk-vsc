...
<li><b>«Быстрая анкета для менеджера» (новое, каркас):</b> на этапе «Начало оформления» — карточка с полями ФИО + страна поездки + даты оформления и кнопкой «Отправить». По отправке создаётся задача в amoCRM на текущего ответственного по сделке (срок — сразу), а данные сохраняются для подстановки в опросник. Список полей уточняется (Ксюша/РОП).</li>
</ul>`;
