...
const LK_TESTER_SPEC = {
  ...
  stageSees: {
    0: [
      "Карточка «Данные для подготовки договора» (скрыта за кнопкой «Заполнить данные для договора» — разворачивается на том же экране, кнопка «Скрыть» сворачивает): даты поездки + email + страховка (своя/оформить у нас) + внутренний паспорт (ФИО, прописка, серия/номер). По отправке — задача в amoCRM на ответственного; даты и страховка автоподставляются в опросники всех заявителей.",
  ...
  screens: [
    { stage: 0, title: "0 · Начало оформления", sees: [
      "Карточка «Данные для подготовки договора» (скрыта за кнопкой «Заполнить данные для договора», разворачивается на том же экране, «Скрыть» сворачивает): даты поездки + email + страховка (своя/оформить у нас) + внутренний паспорт (ФИО, прописка, серия/номер) → задача в amoCRM на ответственного; даты и страховка автоподставляются в опросники всех заявителей (корректировка Зайцевой).",
      "Загрузка двух паспортов (внутренний + загран) на каждого заявителя; кнопка «Заполнить опросник».",
