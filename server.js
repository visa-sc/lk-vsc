...
app.get("/", (req, res, next) => {
  ...
  // work.voyotravel.ru (бывш. admin., переименован 20.08.2026; admin. — 301 на work.)
  // обслуживает ОТДЕЛЬНЫЙ сервис kateadmin (:3002, nginx ведёт
  ...
  try {
    ...
    try {
      ...
      if (pk && inviterPk && inviterPk !== pk && !b.referredBy[pk]) {
        const rNew = (b.referredBy[pk] = { refPhone: inviterPk, code: promoCodeRaw, ts: Date.now(), status: "pending", src: "register" });
        brefSave();
        bloyCreditWelcome(pk, rNew, "Приветственные баллы за регистрацию по приглашению");
        console.log(`BETA-LOYALTY: регистрация по промокоду ${promoCodeRaw} — ${bloyMask(pk)}, приветственные ${BLOY_REF_FRIEND}`);
      }
    } catch (_) {}
    return res.json({
      success: true,
...
(function schedulePlatrumPrewarm() {
  ...
  setTimeout(warm, 90 * 1000);
  setInterval(warm, 6 * 3600 * 1000);
})();
...
// SUCCESS — сделка успешно завершена. ТОЛЬКО по этим двум статусам начисляется
// кэшбэк (решение Андрея 19.08): 138231/142 «Успешно реализовано (для ВНЖ)» в
// Отделе Продаж и 1309524/142 «Успешно реализовано» в Отделе по работе с Клиентами.
// «Пакет документов готов» (Отдел Оформления) из набора УБРАН — это не завершение
// сделки, а промежуточный этап, и сделка уходила с него дальше по процессу.
...
const BLOY_REF_INVITER = 2000;          // пригласившему — ПОСЛЕ оплаты друга
const BLOY_REF_FRIEND = 2000;           // другу — СРАЗУ при привязке промокода (решение Андрея 19.08)
...
// Приветственные баллы ДРУГУ — сразу в момент привязки промокода (решение Андрея
// 19.08): чтобы человек увидел баллы на счету и мог сразу ими воспользоваться.
// Идемпотентно: метка welcomeTs на привязке не даёт начислить дважды.
// Уже начисляли приветственные? Метка на привязке — быстрый путь; журнал —
// страховка для привязок, созданных до появления метки (иначе доначислили бы дважды).
function bloyHasWelcome(friendPk, r) {
  if (r && r.welcomeTs) return true;
  const old = brefLedger(friendPk).find((e) => e.type === "welcome" && (Number(e.points) || 0) > 0);
  if (old) { if (r) r.welcomeTs = old.ts; return true; }
  return false;
}
function bloyCreditWelcome(friendPk, r, note) {
  if (!r || bloyHasWelcome(friendPk, r)) return false;
  const b = brefLoad(), now = Date.now();
  (b.ledger[friendPk] = b.ledger[friendPk] || []).push({ ts: now, type: "welcome", points: BLOY_REF_FRIEND, note: note || "Приветственные баллы по приглашению" });
  r.welcomeTs = now; brefSave();
  return true;
}
// Баллы ПРИГЛАСИВШЕМУ — только после оплаты друга (сделка в платёжных статусах и
// заполнен «Бюджет»). Приветственные другу к этому моменту уже начислены; если
// привязка старая и их нет — доначисляем здесь же.
function bloyCreditReferral(friendPk, r, note) {
  const b = brefLoad(), now = Date.now();
  bloyCreditWelcome(friendPk, r);
...
async function runBetaLoyalty(trigger) {
  ...
  try {
    ...
    // Возвраты берём БЕЗ фильтра по дате оплаты: если поле даты на возврате
    // почистили, сделка всё равно должна выйти из программы.
    const refundLeads = await fetchByStatuses(BLOY_REFUND_STATUSES);
    const refundIds = new Set(refundLeads.map((l) => Number(l.id)));
    ...
    // Реестр «однажды успешных» сделок (решение Андрея 19.08). Сделка, попавшая в
    // успешный статус, остаётся в программе НАВСЕГДА — даже если её потом двинули
    // дальше по процессу и она вышла из успешного статуса. Иначе у клиента исчезали
    // уже показанные ему баллы. Из реестра сделка уходит ТОЛЬКО при возврате.
    const registry = Object.assign({}, (bloyLoad() || {}).qualifiedDeals || {});
    const remember = (l) => {
      const price = Number(l.price) || 0; if (price <= 0) return;     // бюджет сделки
      const cid = mainCid(l); const info = cid ? cmap[cid] : null;
      if (!cid || !info || !info.phones.length) return;
      const ts = payTs(l) || Number(l.updated_at || l.created_at || 0) * 1000;   // дата оплаты = дата начисления
      registry[l.id] = { id: l.id, name: l.name || "", price, ts, contactId: cid, contactName: info.name, phones: info.phones };
    };
    successLeads.forEach(remember);
    // Пока сделка видна среди оплаченных — освежаем сумму, телефоны и имя контакта.
    paidLeads.forEach((l) => { if (registry[l.id]) remember(l); });
    // Возврат — вычитаем сделку из программы вместе с начисленными за неё баллами.
    let revokedDeals = 0;
    Object.keys(registry).forEach((id) => { if (refundIds.has(Number(id))) { delete registry[id]; revokedDeals++; } });
    ...
    Object.values(registry).forEach((d) => {
      priced++;
      const acc = accounts[d.contactId] || (accounts[d.contactId] = { contactId: d.contactId, name: d.contactName || "", phones: d.phones, earned: 0, spend: 0, deals: [] });
      if (!acc.name && d.contactName) acc.name = d.contactName;
      acc.deals.push({ id: d.id, name: d.name || "", price: d.price, points: 0, rate: 0, ts: d.ts, day: _mskDayKey(d.ts) });
    });
    ...
    bloySave({ ts: Date.now(), rate: BLOY_RATE, tiers: BLOY_TIERS, start: BLOY_START_DAY,
      accountsCount: Object.keys(accounts).length, totalEarned, scanned: Object.keys(registry).length,
      qualified: priced, paidScanned: paidLeads.length, paidPhones: paidUniq, qualifiedDeals: registry,
      successNow: successLeads.length, refundScanned: refundIds.size,
      durationMs: Date.now() - t0, accounts });

    const b = brefLoad(); let credited = 0, revoked = 0, welcomed = 0; const now = Date.now();
    Object.keys(b.referredBy).forEach((friendPk) => {
      const r = b.referredBy[friendPk]; if (!r) return;
      const paid = hasPaid(friendPk);
      // Привязка есть, а приветственных нет — значит она из времён, когда баллы
      // давали только после оплаты. Доначисляем другу сразу, без всяких условий.
      if (!bloyHasWelcome(friendPk, r)) {
        ...
        r.welcomeTs = now; welcomed++;
      }
      if (paid && r.status !== "qualified") {
        // Друг оплатил — начисляем ПРИГЛАСИВШЕМУ (приветственные другу уже у него).
        ...
        // Оплаты больше нет (возврат) — снимаем только баллы пригласившего.
        // Приветственные друг оставляет себе: они даны за регистрацию, а не за оплату,
        // и он мог их уже потратить.
    ...
    if (credited || revoked || welcomed) brefSave();
    console.log(`BETA-LOYALTY [${trigger || "cron"}]: в реестре ${Object.keys(registry).length} сделок (сейчас в успешном статусе ${successLeads.length}, снято возвратом ${revokedDeals}), аккаунтов ${Object.keys(accounts).length}, начислено ${totalEarned}, рефералов +${credited}/-${revoked}, приветственных ${welcomed}, ${Date.now() - t0}ms`);
...
app.post("/beta/api/loyalty/refclaim", (req, res) => {
  ...
  const rNew = (b.referredBy[pk] = { refPhone: inviterPk, code, ts: Date.now(), status: "pending" }); brefSave();
  bloyCreditWelcome(pk, rNew, "Приветственные баллы за регистрацию по приглашению");
  return res.json({ ok: true, welcome: BLOY_REF_FRIEND });
});
...
app.post("/admin/api/loyalty/beta/promo", requireVscAccess, (req, res) => {
  ...
  // Приветственные другу — сразу, независимо от оплаты.
  const welcomeNow = bloyCreditWelcome(pk, r, "Приветственные баллы по приглашению (промокод внесён вручную)");
  if (bloyHasPaid(pk)) {
  ...
  return res.json({ success: true, credited: false, welcome: welcomeNow ? BLOY_REF_FRIEND : 0,
    message: (welcomeNow ? "Клиенту начислено " + BLOY_REF_FRIEND + " приветственных баллов. " : "Промокод зафиксирован. ")
      + "Баллы пригласившему начислим, как только клиент оплатит." });
});
...
// Отдельные эндпоинты, потому что в ЛК действует Фаза 2 авторизации: телефон
// берём ТОЛЬКО из подписанной cookie-сессии voyo_sess, ?phone= здесь не
// принимаем вообще. Данные и логика — те же, что у /app и /loyalty.
//
// РУБИЛЬНИК: LK_LOYALTY=0 в .env + pm2 restart → раздел мгновенно исчезает из
// ЛК (эндпоинты отдают 404, /api/me возвращает loyalty:false, кнопка не
// рисуется). Откат без правки кода и без деплоя.
const LK_LOYALTY_ON = process.env.LK_LOYALTY !== "0";
// Возвращает телефон клиента или null (ответ клиенту уже отправлен).
function lkLoyPhone(req, res) {
  if (!LK_LOYALTY_ON) { res.status(404).json({ success: false, enabled: false }); return null; }
  const phone = clientPhoneFromSession(req);
  if (!phone) { res.status(401).json({ success: false, message: "Нужен вход" }); return null; }
  return phone;
}
app.get("/cabinet/api/loyalty", (req, res) => {
  res.set("Cache-Control", "no-store");
  const phone = lkLoyPhone(req, res); if (!phone) return;
  const r = bloyBuildCard(phone);
  return res.json({ success: true, card: r.card, referral: r.referral });
});
app.post("/cabinet/api/loyalty/redeem", (req, res) => {
  const phone = lkLoyPhone(req, res); if (!phone) return;
  const pk = bloyPk(phone);
  const points = Math.floor(Number((req.body && req.body.points) || 0));
  const active = bloyActiveRedeem(pk);
  ...
  if (points > balance) return res.json({ ok: false, reason: "not_enough", balance });
  const id = "r" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  brefRedeems()[id] = { id, pk, points, comment: "", ts: Date.now(), status: "new", src: "lk" };
  brefSave();
  console.log(`BETA-LOYALTY redeem-request (ЛК): ${bloyMask(pk)} — ${points} б.`);
  return res.json({ ok: true, request: { id, points, ts: Date.now() } });
});
app.post("/cabinet/api/loyalty/redeem/cancel", (req, res) => {
  const phone = lkLoyPhone(req, res); if (!phone) return;
  const pk = bloyPk(phone);
  const r = brefRedeems()[String((req.body && req.body.id) || "")];
  if (!r || r.pk !== pk) return res.status(404).json({ ok: false });   // чужую заявку не отменить
  if (r.status !== "new") return res.json({ ok: false, reason: "already_decided", status: r.status });
  r.status = "canceled"; r.decidedTs = Date.now(); brefSave();
  return res.json({ ok: true });
});
...
// ── VOYO eSIM (/esim, создан 02.09.2026): тестовая витрина туристических eSIM
// под брендом VOYO. Поставщик MobiMatter спрятан за интерфейсом в esim.js
// (на объёме заменится на eSIM Go/Maya без переделки витрины). Цены: закупка $
// × курс ЦБ × наценка. Пока без эквайринга: кнопка «Купить» = заявка с
// телефоном → письмо менеджеру. Клиентский ЛК не затрагивает; потом витрина
// встанет в кабинет отдельной вкладкой.
// ── VOYO Экскурсии (/excursion, создан 05.09.2026): собственный сервис
// бронирования экскурсий под брендом VOYO. Поставщик Sputnik8 спрятан за
// интерфейсом в excursion.js: каталог, реальное расписание, билеты и цены
// тянем по API, витрина и оформление — наши. Последний шаг (подтверждение и
// оплата) идёт в защищённой форме поставщика, встроенной в нашу страницу и
// преднастроенной выбором клиента, — до тех пор, пока Sputnik8 не выдаст
// права на POST /orders (тогда включаем SPUTNIK8_BOOK_API=1 и оформление
// становится полностью нашим). Клиентский ЛК и /admin не затрагивает.
require("./excursion").mount(app, { sendMail: (o) => mail.sendMail(o) });

// Раздел «Карты» кабинета и страница /cards: виртуальная карта (оформление у
// партнёра по нашей реферальной метке, две кнопки) и именная карта зарубежного
...
require("./esim").mount(app, {
  ...
  // и обращается сюда только когда её ещё нет.
  lkClient: async (req) => {
    const phone = clientPhoneFromSession(req);
...
<li><b>«eSIM» — раздел меню кабинета (с 04.09.2026):</b> витрина туристических eSIM (VOYO mobile) прямо в кабинете, <b>на всех этапах</b>. Открывается поверх кабинета и показывает ту же страницу voyotravel.ru/esim в режиме встраивания: поиск страны, пакеты, оплата картой или СБП, промокоды. Внутри — переключатель «Купить eSIM» / «Мои eSIM» (QR, остаток трафика, срок, продление); почту повторно не спрашиваем — берём из карточки amoCRM по телефону сессии или из прошлой покупки, связка хранится в .esim/lkbind.json. Реферальной программы eSIM и списания её бонусов в кабинете НЕТ — они остаются только на самой /esim. Оплата уводит на страницу банка целой вкладкой. Отдельная страница /esim при этом не меняется.</li>
<li><b>«Карты» — раздел меню кабинета (с 12.09.2026):</b> банковские карты для оплат за границей, <b>на всех этапах</b>. Открывается поверх кабинета и показывает страницу voyotravel.ru/cards в режиме встраивания, две вкладки. <b>«Виртуальная»</b> — карты МИР и Visa (МИР бесплатно, Visa 1 990 ₽ вместо 4 990 ₽), выпуск за минуты, Apple Pay и Google Pay, пополнение по СБП и в USDT, лимиты и «для чего подходит» тезисами; оформление у партнёра по нашей реферальной метке двумя кнопками — «Оформить в Телеграме» и «Оформить онлайн», своей формы нет, переходы пишутся в .cards/clicks.json. <b>«Физическая»</b> — именная карта Visa/Mastercard зарубежного банка на 5 лет, тарифы Gold от 14 900 ₽, Priority 19 900 ₽, Infinite 49 000 ₽, порядок оформления и документы; кнопка «Оставить заявку» отправляет заявку в один тап (телефон из сессии voyo_sess, письмо на director@visa-sc.ru, копия в .cards/leads.json), рядом кнопка «Позвонить». На отдельной странице /cards вне кабинета заявка спрашивает телефон.</li>
<li><b>«Туры» — раздел меню кабинета (с 11.09.2026):</b> экскурсии по всему миру прямо в кабинете, <b>на всех этапах</b>. Открывается поверх кабинета и показывает страницу voyotravel.ru/tours в режиме встраивания: каталог по городам и странам с фильтрами, карточка экскурсии (галерея, программа, что входит, гид, отзывы, место встречи), реальное расписание и билеты с ценами в рублях. Поставщик — Sputnik8, спрятан за интерфейсом: ключ живёт только на сервере, цены приводим к рублям и берём по базовому билету ближайшего сеанса. Подтверждение и оплата открываются отдельной вкладкой на стороне партнёра (у него у формы жёстко target="_blank"), после возврата клиент видит наш экран «Оформление открылось в новой вкладке». Есть кнопка «Оформить через менеджера» — заявка письмом на director@visa-sc.ru. Отдельная страница /tours при этом не меняется.</li>
<li><b>«Баллы» — раздел меню кабинета (с 11.08.2026):</b> внизу по центру меню-«пилюля»; с 12.09.2026 в ней пять разделов — «Визы», «eSIM», «Карты», «Туры» и «Баллы» (подпись видна у открытого, остальные — иконками) (до 11.08 была круглая кнопка-человечек), <b>на всех этапах</b>. Плюс карточка-оффер «Пригласите друга и получите 4 000 ₽!» с кнопкой «Подробнее»: на десктопе слева под «Вашими обращениями», на мобиле — под рабочей областью перед блоком «Нужна помощь?». Открывает экран: карта (баланс, статус со ставкой, шкала «Заказано на N ₽» по всей лестнице статусов), реферальный код с кнопками WhatsApp/Telegram и кнопка «Показать историю». Все пояснения — под иконкой «i» у названия статуса. Программа с 11.08.2026, считаются сделки с датой оплаты от этого дня, сумма — поле «Бюджет» amoCRM. Кэшбэк только за успешно завершённые сделки (возврат аннулирует баллы): Базовый 0%, Titanium 5% от 20 000 ₽, Silver 7% от 50 000 ₽, Gold 10% от 100 000 ₽, Platinum 12% от 150 000 ₽, Black 15% от 300 000 ₽, Infinite 20% от 500 000 ₽; 1 балл = 1 ₽, оплатить баллами можно до 30% услуги. За приглашённого друга по 2 000 баллов обоим — в момент его оплаты (промокод вводится при регистрации либо фиксируется сотрудником в админке); возврат по его сделке сторнирует начисление. Списывает баллы менеджер при оформлении — формы списания в ЛК нет. Телефон берётся из сессии voyo_sess. Полная механика — во вкладке «Лояльность» раздела «Логика и справочники». Раздел выключается рубильником LK_LOYALTY=0 в .env — тогда в ЛК нет ни меню, ни карточки-оффера.</li>
...
const KB_LOGIC_STAGES = [
  { key: "Начало оформления", sees: "Загрузка 2 паспортов (внутренний + загран) — обеим визам (Япония как Шенген, решение Андрея 03.07). Возможность заполнить опросник (паспорта можно загрузить и до опросника — тогда спросим ФИО).", dep: ["Этап определяется статусом сделки в amoCRM (см. «Карту статусов»).", "Опросник: Шенген, Кипр (шенгенский опросник со страной «Кипр» — корр. Ксении М. 22.07) или Япония — по выбранной визе/услуге.", "Шенген-опросник: «Дата рождения» (обязательное, после ФИО — корр. Насти 10.07, просьба ОРК); «Город подачи» + «Прописка в паспорте РФ в городе подачи?» (при «Нет» — город прописки) и блок «Данные принимающей стороны» при цели «Частный визит» (корр. Риты) — все в PDF-выгрузке."] },
...
const LK_CHANGELOG = [
  { date: "12.09.2026", title: "Меню кабинета: новый раздел «Карты»", by: "задача Андрея", points: [
    "В нижнем меню кабинета теперь пять разделов: «Визы», «eSIM», «Карты», «Туры» и «Баллы». Подпись показывается у открытого раздела, остальные — иконками: пять подписей в «пилюлю» не помещаются.",
    "«Карты» открывает страницу voyotravel.ru/cards в режиме встраивания. Две вкладки: «Виртуальная» и «Физическая».",
    "Виртуальная — карты МИР и Visa с выпуском за минуты, привязкой к Apple Pay и Google Pay, пополнением по СБП и в USDT. Оформление у партнёра по нашей реферальной метке: две кнопки, «Оформить в Телеграме» и «Оформить онлайн». Своей формы нет.",
    "Физическая — именная карта Visa/Mastercard зарубежного банка на 5 лет, три тарифа (Gold, Priority, Infinite). Кнопка «Оставить заявку» отправляет заявку менеджеру в один тап: телефон берётся из сессии кабинета, форму клиент не заполняет. Рядом кнопка «Позвонить». Заявка уходит письмом на director@visa-sc.ru.",
    "Раздел доступен на всех этапах. Страница /cards открывается и отдельно, вне кабинета: там заявка спрашивает телефон."
  ] },
  { date: "11.09.2026", title: "Меню кабинета: новый раздел «Туры»", by: "задача Андрея", points: [
    "В нижнем меню кабинета теперь четыре раздела: «Визы», «eSIM», «Туры» и «Баллы». «Туры» — экскурсии по всему миру: каталог с ценами в рублях, отзывами и рейтингом, выбор даты, времени и билетов, бронирование прямо в кабинете.",
    "Раздел открывается поверх кабинета и показывает ту же страницу voyotravel.ru/tours в режиме встраивания, без своей шапки и подвала. Доступен на всех этапах.",
    "Подтверждение и оплата брони открываются отдельной вкладкой на стороне партнёра (по-другому он не умеет), после чего клиент возвращается в кабинет. Ваучер приходит на почту, указанную при бронировании."
  ] },
  { date: "04.09.2026", title: "eSIM в кабинете: меню не мешает карточке пакета", by: "замечание Андрея", points: [
    "Пока открыта карточка пакета, нижнее меню кабинета плавно уезжает вниз: раньше оно ложилось поверх кнопки «Купить» и обрезало низ карточки. Закрыли карточку — меню вернулось.",
    "Сортировка пакетов по умолчанию — «Дешевле» (самые доступные сверху), чтобы большой ценник не отпугивал."
  ] },
  { date: "04.09.2026", title: "eSIM в кабинете: «Мои eSIM» без повторного входа и сортировка пакетов", by: "замечания Андрея", points: [
    "В разделе «eSIM» появился переключатель «Купить eSIM» / «Мои eSIM». Во второй вкладке — купленные eSIM: QR-код, остаток гигабайтов, срок, продление в клик.",
    "Почту больше не спрашиваем второй раз: клиент уже вошёл в кабинет по телефону, почту берём из его карточки amoCRM или из прошлой покупки и запоминаем. Если покупок на эту почту нет — показываем пустой список и ссылку «Покупали на другую почту?», а не форму входа. Ссылка из письма возвращает человека сразу в кабинет, в раздел eSIM.",
    "Витрина перенесена в кабинет целиком: шаги «Как это работает» и частые вопросы на месте. Свой логотип и подвал eSIM-страницы скрыты, блок обратной связи — общий кабинетный (WhatsApp, Telegram, звонок), одинаковый в «Визах» и в «eSIM».",
    "Над списком пакетов — шильдик сортировки: «Выгодные» (лучшая цена за гигабайт, по умолчанию), «Подешевле», «Больше ГБ». Выбор запоминается в браузере. Работает и в кабинете, и на отдельной странице /esim.",
    "Служебные тестовые пакеты за один цент убраны из витрины совсем — купить их случайно нельзя."
  ] },
  { date: "04.09.2026", title: "Меню кабинета: раздел «eSIM», «Визы» и «Баллы»", by: "задача Андрея", points: [
    "В нижнем меню три раздела: «Визы» (было «Обращения»), «eSIM» и «Баллы» (было «Бонусы»).",
    "«eSIM» открывает поверх кабинета витрину VOYO mobile — ту же страницу /esim, но без своей шапки, вопросов и подвала: поиск страны, пакеты, оплата картой или СБП, промокоды. Промокоды работают, реферальной программы eSIM и списания её бонусов в кабинете нет — они остались на отдельной странице /esim.",
    "Внутри раздела переключатель «Купить eSIM» / «Мои eSIM»: во второй вкладке — купленные eSIM с QR-кодом, остатком гигабайтов, сроком и продлением. Почту второй раз не спрашиваем: клиент уже вошёл в кабинет по телефону, почту берём из его карточки amoCRM или из прошлой покупки. Если покупок на неё нет — просто пустой список и ссылка «Покупали на другую почту?», а не форма входа. Ссылка из письма возвращает человека сразу в кабинет, в раздел eSIM.",
    "Над списком пакетов — шильдик сортировки: «Выгодные» (лучшая цена за гигабайт, по умолчанию), «Подешевле», «Больше ГБ». Выбор запоминается в браузере. Служебные тестовые пакеты из витрины убраны совсем.",
    "Витрина подгружается только когда клиент открыл раздел; на скорость кабинета это не влияет. Оплата уводит на страницу банка целой вкладкой, а не внутри рамки.",
    "Разделы «Визы» и «Баллы» работают как раньше — изменились только подписи и порядок кнопок."
  ] },
  ...
  { date: "19.08.2026", title: "Приветственные баллы другу — сразу при регистрации", by: "решение Андрея", points: [
    ...
    "При возврате по сделке приглашённого снимаются только баллы пригласившего. Приветственные друг оставляет себе — они даны за регистрацию, а не за оплату, и могли быть уже потрачены."
  ] },
  { date: "19.08.2026", title: "Баллы за два статуса и больше не пропадают", by: "решение Андрея", points: [
    "Кэшбэк начисляется по двум статусам: «Успешно реализовано (для ВНЖ)» (Отдел Продаж) и «Успешно реализовано» (Отдел по работе с Клиентами). «Пакет документов готов» (Отдел Оформления) кэшбэк больше не даёт — это промежуточный этап.",
    "Сделка, попавшая в успешный статус, остаётся в программе навсегда: баллы за неё сохраняются, даже если сделку потом перевели дальше по процессу. Раньше при таком переводе баллы у клиента молча обнулялись.",
    "Баллы за сделку снимаются только при её попадании в «Возврат». Оплатит снова и дойдёт до успешного статуса — начислим заново."
  ] },
  ...
  { date: "11.08.2026", title: "Оффер прижат к «Вашим обращениям», строка в одну линию, меню выше", by: "замечания Андрея", points: [
    "Карточка-оффер на десктопе встала ВПЛОТНУЮ под «Ваши обращения»: первая строка сетки сжата по содержимому, раньше она тянулась за высоким «Статусом обращения» и оффер уезжал вниз колонки.",
    "Текст «Пригласите друга и получите 4 000 ₽!» помещается в одну строку на любой ширине; вернулся прежний фон карточки — мягкий переход из белого в голубой.",
    "Меню приподнято над нижним краем экрана и больше не опускается при сжатии: в свёрнутом виде меняются только размеры, отступ снизу постоянный."
  ] },
  { date: "11.08.2026", title: "Меню: «Обращения», сжатие при скролле; оффер — в стиле карточки ЛК", by: "замечания Андрея", points: [
    "Первый раздел меню называется «Обращения» (было «Визы»), иконка прежняя.",
    "При прокрутке меню сжимается: становится ниже и уже, подписи скрываются — остаются только иконки. Вернулись наверх — меню разворачивается обратно. Работает и в кабинете, и на экране бонусов.",
    "Карточка-оффер больше не выделяется голубым градиентом и крупным синим «4 000 ₽»: теперь это обычная белая карточка кабинета (тот же фон, рамка и скругление, что у «Ваших обращений») с жирной строкой «Пригласите друга и получите 4 000 ₽!» и кнопкой «Подробнее».",
    "На десктопе карточка встала вплотную ПОД «Ваши обращения»: «Статус обращения» растянут на обе строки сетки, поэтому оффер больше не уезжает вниз страницы. Правила раскладки включаются только вместе с карточкой — при выключенной программе сетка кабинета прежняя.",
    "На мобиле без изменений: карточка под рабочей областью, перед блоком «Нужна помощь?»."
  ] },
  { date: "11.08.2026", title: "Меню кабинета «Визы / Бонусы» и оффер «Пригласите друга»", by: "задача Андрея", points: [
    "Вместо круглой кнопки-человечка внизу появилось МЕНЮ-«пилюля» с двумя разделами — «Визы» (сам кабинет) и «Бонусы» (экран бонусной программы). Сделано по образцу меню в приложении /app: полупрозрачный фон с блюром, у активного раздела — синий овал.",
    "«Бонусы» открывают отдельный экран поверх кабинета (карта, прогресс, приглашение друзей, история); «Обращения» возвращают к списку обращений. Esc тоже возвращает. Содержимое кабинета при этом не меняется и не перерисовывается.",
    "Добавлена карточка-оффер «Пригласите друга и получите 4 000 ₽!» с кнопкой «Подробнее» (ведёт в раздел «Бонусы»). На десктопе она стоит слева под блоком «Ваши обращения», на мобиле — под рабочей областью, перед блоком «Нужна помощь?».",
    "И меню, и карточка-оффер показываются только при включённой программе (рубильник LK_LOYALTY): при выключении исчезают вместе с разделом."
  ] },
  { date: "10.08.2026", title: "Кнопка «Бонусы» — убрана синяя точка", by: "замечание Андрея", points: [
    "С круглой кнопки-человечка в правом нижнем углу кабинета убран синий индикатор-точка: он ничего не означал (не счётчик новых баллов и не уведомление) и вызывал у клиента вопросы.",
    "Сама кнопка, шторка и содержимое бонусного раздела не изменились."
  ] },
  { date: "10.08.2026", title: "«Бонусы» в ЛК — бонусная карта клиента (кнопка-человечек)", by: "задача Андрея", points: [
    "В правом нижнем углу кабинета появилась круглая кнопка с человечком — открывает шторку «Бонусная программа». Видна на всех этапах, существующие блоки ЛК не двигает и ничего в них не меняет. Синяя точка-индикатор на кнопке убрана (10.08) — она ничего не означала и сбивала с толку.",
    ...
    "Кэшбэк — ТОЛЬКО за успешно завершённые сделки, и только по двум статусам: «Успешно реализовано (для ВНЖ)» (Отдел Продаж) и «Успешно реализовано» (Отдел по работе с Клиентами). Попала в любой из них — баллы начислены и остаются за клиентом, даже если сделку потом двинули дальше по процессу. Снимаются баллы только при попадании сделки в «Возврат».",
  ...
  { date: "22.07.2026", title: "«Виза на Кипр» в выборе визы/услуги — шенгенский опросник", by: "корректировка Ксении М.", points: [
    ...
    "Для Кипра открывается ПОЛНЫЙ шенгенский поток: выбор числа заявителей, режим «разослать по SMS», сам опросник (заголовок «Опросник на визу на Кипр»), PDF-выгрузка, документы по этапам. В поле «В какую страну запрашивается виза» страна «Кипр» подставлена автоматически (клиент может изменить).",
    "Бонус, работавший и раньше: при «Страна оформления/услуга» = Кипр в CRM клиенту на «Первичном сборе» запрашивается «Электронное фото»; в подсказках о сроке справок Кипр уже упоминался.",
    "amoCRM, Япония и существующий Шенген-поток не затронуты."
  ] },
  { date: "21.07.2026", title: "ЛК-скин: починена мобильная всплывашка «Обращения»", by: "доводка дизайна (баг-репорт Андрея)", points: [
    "На мобильной ширине по нажатию на шильдик «Обращение от …» экран мутнел, но панель со списком обращений не появлялась — она оказывалась ПОД подложкой.",
    "Причина: backdrop-filter, добавленный скином на обёртку .wrap, создавал stacking context/containing block — фиксированная панель #obrCard (z-index 1001) переставала подниматься над подложкой (z-index 1000). Blur с .wrap убран (фон стал чуть плотнее — визуально неотличимо), всплывашка снова открывается поверх.",
    "Только CSS; логика и остальные элементы не менялись."
  ] },
  { date: "21.07.2026", title: "ЛК-скин: логотип в шапке кабинета — снова без белой подложки", by: "доводка дизайна (задача Андрея)", points: [
    "После включения «воздушного» скина вокруг PNG-логотипа VOYO в шапке кабинета появлялся белый прямоугольник: фикс-фон body (background-attachment: fixed) в Chrome не смешивается с mix-blend-mode: multiply логотипа.",
    "Градиент перенесён с body на фиксированный слой body.lk-skin::before — multiply снова «растворяет» белую подложку (проверено пиксельно и на живом проде). Виден только мягкий голубой ореол самого фона.",
    "Только CSS: разметка, логика и остальные страницы не менялись."
  ] },
  { date: "21.07.2026", title: "Клиентский ЛК: «воздушный» дизайн в стиле Apple (как /vsc)", by: "задача Андрея (только внешний вид)", points: [
    "Кабинет (cabinet.html) и страница входа (index.html) получили скин в духе /vsc: мягкий градиентный фон, тонкие границы и двойные мягкие тени карточек, деликатные градиенты на кнопках c hover-подъёмом, фокус-кольца полей, спокойная типографика (letter-spacing), блюр подложек модалок. Фирменный акцент #3589BD сохранён.",
    "Реализовано ТОЛЬКО CSS-надстройкой body.lk-skin в конце стилей — разметка, размеры, JS-логика, вход (SMS/Face ID), опросники и логика документов не менялись ни на строчку. Откат = убрать класс lk-skin с body.",
    "Доводка: логотип VOYO в шапке кабинета снова без белой подложки — градиент перенесён с body на фикс-слой ::before, чтобы mix-blend-mode: multiply у PNG-логотипа продолжал работать (с background-attachment: fixed на body Chrome не смешивает blend с фоном).",
    "Опросник (/questionnaire) и /beta это изменение не затрагивает."
  ] },
  ...
  directions: {
    schengen: {
      ...
      qrecs: [
        ...
        "«Дата рождения» (добавлена 10.07 по просьбе ОРК) пока вводится вручную. Отложенная идея Насти: автозаполнять её из скана паспорта (распознавание MRZ/OCR) — с 22.08 /scanner — рабочий сервис (MRZ разбирается на сервере с контрольными цифрами, кириллицу читает модель по фото; снимок уходит на сервер и в Anthropic, хранится 3 дня). Рекомендация: подключить, когда дойдут руки."
      ],
      docRecs: [
...
function vscParseMonth(rows) {
  ...
  let ctrl = null;
  {
    ...
    // «Количество сделок из повторных…» (до июня) / «Кол-во сделок из повторных…»
    // (с июля лист переименовали) — ловим оба варианта по ["сделок","повторных"];
    // колонку «Выручка по повторным заявкам» не заденет (нет слова «сделок»).
    // Из знаменателя исключаем новые июльские колонки «Кол-во сделок из новых/
    // повторных заявок» — иначе сделки считались бы дважды.
    const repCols = colsAll(["сделок", "повторных"]);
    const dealCnt = colsAll(["кол-во сделок"], ["новых", "повторн"]);
...
// vatStart — с какого МЕСЯЦА (ГГГГ-ММ) юрлицо платит НДС (Андрей: у Альты и Комисаренко
// НДС начался с мая 2026). НДС-база квартала = выручка месяцев ≥ vatStart.
const TAXLEG_ENTITIES = [
  { key: "alta", name: "Альта Профит", rate: 0.015, vatStart: "2026-05" },
  { key: "kom", name: "ИП Комисаренко", rate: 0.03, vatStart: "2026-05" },
  { key: "pan", name: "ИП Панфилова", rate: 0.03, vatStart: "2026-01" },
    ...
    quarters: {
      ...
      "2026-Q2": {
        ...
        buh: {
          ...
          pan: { tax: 370590, vat: 353806, income: 12352978 }, // НДС скорректирован бухгалтером 22.07 (617 650 → 588 237 → 353 806)
          akg: { tax: 30313, vat: 448764, income: 11832085, expense: 11630000 } // НДС скорректирован бухгалтером 22.07 (было 591 604)
        }
      }
    }
  };
...
app.get("/api/me", (req, res) => {
  ...
  // loyalty — включён ли раздел «Бонусы» в ЛК (рубильник LK_LOYALTY в .env).
  // Отдаём здесь, чтобы ЛК не делал ради этого отдельный запрос при загрузке.
  return res.json({ success: true, phone, loyalty: LK_LOYALTY_ON });
});
...
function buildQuestionnaireHtml({ phone, leadId, countryService, applicantIndex = 1, totalApplicants = 1, prevApplicantName = "", prefill = null, isEdit = false, applicantCount = 0, visaType = "", shareToken = "", isMixed = false, selfFillCount = 0, selfStep = 0, existingFios = [] }) {
  ...
  const titleText = visaType === "Шенгенская виза"
    ...
    : visaType === "Виза на Кипр"
      ? "Опросник на визу на Кипр"
...
${mixedFieldsHtml}
    ...
    <div class="field">
      ...
      <select name="visaCountry" required>
        ...
        <option value="Кипр"${visaType === "Виза на Кипр" ? " selected" : ""}>Кипр</option>
...
<script>
  ...
  function update() {
    ...
    // Поддерживаемые опросники в ЛК: Шенген + Кипр (шенгенский опросник,
    // корр. Ксении М. 22.07) + Япония. Для остальных — «Раздел в разработке».
    const isSupported = v === "Шенгенская виза" || v === "Виза на Кипр" || v === "Виза в Японию";
  ...
  document.getElementById("startForm").addEventListener("submit", (e) => {
    e.preventDefault();
    const v = visaSelect.value;
    // Поддерживаемые опросники в ЛК — Шенген, Кипр (шенгенский) и Япония.
    if (v !== "Шенгенская виза" && v !== "Виза на Кипр" && v !== "Виза в Японию") return;
