  ...
  directions: {
    schengen: {
      ...
      docRecs: [
        ...
        "После привязки «Справки с работы или учёбы» к «Роду деятельности» (26.06): на этапе она запрашивается только для «Работа по найму» и «Учащийся». Для «Владелец бизнеса или ИП» / «Самозанятый» / «Пенсионер» / «Безработный» справки с работы на этапе больше нет — их подтверждающие документы только в подобласти «перед подачей». Подтвердить, что это желаемое поведение."
      ],
      questions: [
        ...
        { q: "Род занятий (6 вариантов)", req: "Да", shows: "всегда", effect: "«Работа по найму» → этап «Подготовка документов»: «Справка с работы» (обяз.); «Учащийся» → «Справка с учёбы» (обяз.) — раздельно (корр. Насти 26.06). Также влияет на подобласть «перед подачей» (за 7 дней до записи): «Владелец бизнеса или ИП» → «Документы на ИП»; «Учащийся» → «Справка из учебного заведения» (если справка с учёбы уже загружена — показывается «✓ Уже получена»); «Пенсионер» → «Пенсионное удостоверение/справка»; «Самозанятый» → «Справка о самозанятости» (все обязательные)." },
        { q: "Наименование работодателя/учебной организации", req: "Да", shows: "всегда", effect: "Только данные (вносятся в анкету) + открывает телефон работодателя. На запрос «Справки с работы или учёбы» больше НЕ влияет — справка зависит от «Рода деятельности»." },
    ...
    japan: {
      ...
      docRecs: [
        ...
        "У учащегося справка об учёбе запрашивается дважды: «Справка с учёбы» (этап 1) и «Справка из учебного заведения» (подобласть «перед подачей»). По Шенгену дедуп уже реализован 26.06 (если эквивалент загружен раньше — «✓ Уже получена»); для Японии тот же механизм будет распространён по согласованию (см. корректировку Японии — вопрос Насте по составу документов)."
      ],
      questions: [
...
function buildUploadBlocksForApplicantStats(state, lead, stageIndex) {
  ...
  // справка с работы, учащимся — справка с учёбы (раздельно; раньше была общая «…или учёбы»).
  const occSch = String(state.occupation || "").trim();
  if (occSch === "Работа по найму") {
    stage2.push({ field: "workCert", label: "Справка с работы", optional: true });
  } else if (occSch === "Учащийся") {
