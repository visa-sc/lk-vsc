...
<li><b>Обязательность по статусу (логика Насти, июнь 2026):</b> документы оформления (этапы 1–2) помечены бейджем «по статусу» — открыты для загрузки заранее, но обязательными становятся по статусу сделки в amoCRM и «Дате записи на подачу»: ранние статусы — необязательно; «Запись сделана / Электронное рассмотрение / Оформлен выкуп» — обязательно за ≤7 рабочих дней до подачи; финальные статусы («Исправить / Пакет готов / Принято после ОО / Ожидает передачи / Готовы к личной подаче») — обязательно всё; предохранитель &lt;3 кал. дней до подачи — всё. Паспорта (этап 0) обязательны всегда.</li>
...
<li><b>Вход — идентификация по мессенджеру (новое):</b> если введённого номера НЕТ в amoCRM, перед промокодом клиент отвечает «обращались раньше — Telegram / WhatsApp / Instagram?» → выбор канала → ввод ника (телега/инста, с/без «@») или номера WhatsApp. Сервер ищет контакт в amoCRM (воцап — по номеру; ник — по значению, в т.ч. поле TelegramUsername_WZ) и при совпадении ПРИВЯЗЫВАЕТ новый телефон к найденному контакту, чтобы клиент видел свою историю. Не нашёл — регистрация как нового клиента. Вход по SMS и Face ID/Touch ID при этом НЕ меняется.</li>
<li><b>«Быстрая анкета для менеджера» (новое, каркас):</b> на этапе «Начало оформления» — карточка с полями ФИО + страна поездки + даты поездки и кнопкой «Отправить». По отправке создаётся задача в amoCRM на текущего ответственного по сделке (срок — сразу), а данные сохраняются для подстановки в опросник. Список полей уточняется (Ксюша/РОП).</li>
</ul>`;
...
const KB_DOC_GROUPS = [
  ...
  { stage: "Первичный сбор документов (этап 1) — Япония", byStatus: true, docs: [
...
app.get("/admin/kb/documents", requireStaff, (req, res) => {
  try {
    ...
    b += `<div class="card muted"><b>Когда документ становится обязательным (логика Насти, июнь 2026).</b> Бейдж «обяз./необяз.» — базовая классификация. Фактическая обязательность зависит от <b>статуса сделки в amoCRM + «Даты записи на подачу»</b>: на ранних статусах (Сбор документов для ОО; Принято в работу; Согласование; Сбор оплачен; Ожидает записи вручную/через бота; Эл.документы переданы) документы открыты, но НЕобязательны (помечены «необязательно», можно грузить заранее); на «Запись сделана / Электронное рассмотрение / Оформлен выкуп» документы оформления обязательны за ≤7 рабочих дней до подачи; на «Исправить / Пакет готов / Принято после ОО / Ожидает передачи / Готовы к личной подаче» — обязательны все; предохранитель: &lt;3 календарных дней до подачи — всё обязательно. Паспорта (этап 0) обязательны всегда.</div>`;
...
const KB_LOGIC_STAGES = [
  ...
  { key: "Первичный сбор документов", sees: "Паспорта (этап 0) + первый набор условных документов по ответам опросника.", dep: ["Состав документов зависит от ответов опросника и от поля CRM «Страна оформления/услуга» (электронное фото).", "Обязательность — по статусу сделки в amoCRM + «Дате записи на подачу»: на ранних статусах (Сбор документов для ОО; Принято в работу; Согласование; Сбор оплачен; Ожидает записи вручную/через бота; Эл.документы переданы) документы открыты, но НЕобязательны (помечены «необязательно», можно грузить заранее); на «Запись сделана / Электронное рассмотрение / Оформлен выкуп» документы оформления обязательны за ≤7 рабочих дней до подачи; на «Исправить / Пакет готов / Принято после ОО / Ожидает передачи / Готовы к личной подаче» — обязательны все; предохранитель: <3 календарных дней до подачи — всё обязательно. Паспорта (этап 0) обязательны всегда."] },
...
const LK_TESTER_SPEC = {
  ...
  stageSees: {
    ...
    1: "Паспорта (этап 0) + первый набор условных документов по ответам опросника. Обязательность зависит от статуса сделки и «Даты записи на подачу»: на ранних статусах документы необязательны (помечены «необязательно»), с «Запись сделана» документы оформления обязательны за ≤7 рабочих дней до подачи, на финальных статусах — все (предохранитель: <3 кал. дней — всё). Красная плашка области зеленеет, когда все обязательные на текущем статусе прикреплены.",
  ...
  actions: [
    ...
    { sys: "amoCRM", name: "Задача по «Быстрой анкете для менеджера» (новое)", when: "Клиент отправил быструю анкету (ФИО + страна поездки + даты поездки) на этапе «Начало оформления»", details: "Создаётся задача на текущего ответственного по сделке (срок — сразу) с текстом анкеты; данные сохраняются для подстановки в опросник. Список полей уточняется (Ксюша/РОП). Та же схема выбора ответственного, что и у других задач." },
    { sys: "amoCRM", name: "Привязка телефона к контакту по идентификации (новое)", when: "При регистрации номера, которого нет в amoCRM, клиент указал, что обращался с Telegram/WhatsApp/Instagram, и контакт найден по нику/номеру", details: "Найденному контакту добавляется новый номер телефона (поле PHONE), чтобы клиент видел свою историю. Поиск: воцап — по номеру; ник — по значению (в т.ч. поле TelegramUsername_WZ), с/без «@», любой регистр. Не нашли — регистрируется как новый." }
  ],
  actionRecs: [
  ...
  uploadAreas: [
    ...
    },
    {
      ...
      composition: "Обязательность — по статусу сделки и «Дате записи на подачу» (на ранних статусах необязательно/помечено; с «Запись сделана» — обязательно за ≤7 рабочих дней до подачи; на финальных статусах — всё; <3 кал. дней до подачи — всё). Красно-зелёная плашка одна на блок.",
      color: "Та же плашка: красная → зелёная, когда все обязательные прикреплены.",
  ...
  screens: [
    { stage: 0, title: "0 · Начало оформления", sees: "Карточка «Быстрая анкета для менеджера» (ФИО + страна поездки + даты поездки + «Отправить» → задача в amoCRM на ответственного; данные подставятся в опросник; новое, каркас). Загрузка двух паспортов (внутренний + загран) на каждого заявителя и кнопка «Заполните опросник». Если опросник ещё не заполнен — пустой блок паспортов с модалом ФИО при первой загрузке. Над списком — красная плашка «Необходимо загрузить все документы ниже», зеленеет, когда оба паспорта прикреплены.", dot: "Жёлтая, пока не загружены оба паспорта и/или не заполнен опросник. Если оба паспорта уже загружены — жёлтая снимается, даже если опросник не заполнен." },
    { stage: 1, title: "1 · Первичный сбор документов", sees: "Блоки документов: паспорта (кумулятивно, если ещё не загружены) + первый набор условных документов по ответам опросника. Обязательность — по статусу сделки + «Дате записи на подачу» (на ранних статусах необязательно/помечено; с «Запись сделана» — за ≤7 рабочих дней; на финальных статусах — всё). Красно-зелёная плашка области.", dot: "Жёлтая, пока не загружены обязательные документы текущего статуса." },
    { stage: 2, title: "2 · Подготовка документов", sees: "Документы этапов 0–1 + второй набор (кумулятивно). Обязательность — по статусу сделки + «Дате записи на подачу» (на ранних статусах необязательно/помечено; с «Запись сделана» — за ≤7 рабочих дней; на финальных статусах — всё; <3 кал. дней до подачи — всё). Кнопки «Скорректировать опросник» и «Заполнить ещё опросник». За 7 дней до «Даты записи на подачу» — подобласть «Документы для проверки перед подачей».", dot: "Жёлтая, пока не загружены обязательные текущего статуса в основной области; и отдельно — жёлтая, если активна подобласть «перед подачей» и в ней не загружены все обязательные (даже если основная область уже зелёная)." },
