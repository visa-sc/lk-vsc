...
app.get("/admin/kb/admin-guide", requireStaff, (req, res) => {
  try {
    ...
    b += `<div class="card"><b>Доступы.</b> Сотрудники входят на <b>voyovoyo.ru/team</b> (email + пароль; при первом входе придумываете пароль не короче 6 символов). Сотрудникам видна группа «Работа с ЛК»: «Корректировки ЛК», «Тестировщик ЛК», «Запрос документов», «Опросники (логика)», «Действия», «База знаний ЛК». Статистика и данные клиентов недоступны. Админ входит по коду и видит всё.</div>`;
...
<li>Заявки со статусом «Реализовано» автоматически попадают в «Историю изменений» (База знаний).</li>
</ul>`;
...
const LK_TESTER_SPEC = {
  ...
  stageSees: {
    0: "Загрузка 2 паспортов (внутренний + загран). Доступен опросник (паспорта можно загрузить и до опросника — тогда спросим ФИО).",
    1: "Паспорта (этап 0) + первый набор условных документов по ответам опросника. Все показанные документы обязательны.",
    2: "Документы этапов 0–1 + второй набор условных документов (кумулятивно). За 7 дней до «Даты записи на подачу» появляется подобласть «Документы для проверки перед подачей» (красным — обязательные, синим — необязательные).",
