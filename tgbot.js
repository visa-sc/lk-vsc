...
function dropDominated(list, wide) {
  ...
  list.forEach((p) => {
    ...
    if (!q || p.priceRub < q.priceRub || (p.priceRub === q.priceRub &&
        (n !== q.countries.length ? (wide ? n > q.countries.length : n < q.countries.length) : String(p.id) < String(q.id)))) best[key(p)] = p;
  });
  return list.filter((p) => best[key(p)] === p);
}
...
function blogCodeLetter(rec, refCode) {
  ...
  return "<b>Ваш промокод: <code>" + rec.code + "</code></b>\n" +
    ...
    "• На сайте " + (process.env.ESIM_REF_SITE || "https://voyomobile.ru") + "/esim: введите код в поле «У меня есть промокод» и укажите свою почту. " +
...
async function onIssued(order, n, total) {
  ...
  const how = (total > 1 ? "Каждую eSIM ставьте на свой телефон своим QR-кодом.\n\n" : "") +
    ...
    "<b>Обратите внимание.</b> Перенести eSIM на другой телефон потом нельзя: ставьте её сразу на тот, с которым поедете.\n\n" +
