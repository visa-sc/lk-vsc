...
const SYSTEM = [
  ...
  "- РАЗНООБРАЗЬ форму: не строй каждое сообщение по одной схеме «два абзаца + вопрос». Иногда хватит одной короткой фразы, иногда уместен абзац. Длина и ритм должны меняться от сообщения к сообщению, как у живого человека.",
  "- Пиши грамотно и опрятно: полные слова, пробелы после знаков препинания, без опечаток. Тон и обороты менеджеров перенимай, а их небрежность и ошибки — нет.",
  "- Если вызываешь save_lead: текст, написанный ДО вызова инструмента, клиент не увидит. Весь ответ пиши заново одним сообщением после вызова. Не пиши «жду ответа» и не задавай один вопрос дважды.",
...
async function reply(messages, sessionId, meta) {
  ...
  for (let step = 0; step < 4; step++) {
    ...
    // Клиенту уходит текст только ПОСЛЕДНЕГО ответа модели. Если складывать все
    // шаги, куски до и после вызова save_lead склеиваются в дубль («…?Жду ответа…»).
    let stepText = "";
    for (const b of msg.content) if (b.type === "text") stepText += b.text;
    if (stepText.trim()) text = stepText;
  ...
  // Опрятная пунктуация: пробел после ?!,: перед кириллицей (следы склеек и небрежности)
  text = text.replace(/([?!,:])(?=[А-Яа-яЁё])/g, "$1 ");
