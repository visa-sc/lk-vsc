...
function mount(app, opts) {
  ...
  function readyLetter(order, items, refCode) {
    ...
    return {
      ...
      html: '<div style="font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;max-width:520px;margin:0 auto;color:#16202e">' +
        ...
        'background:#fdf6e7;border:1px solid #f0e2c0;border-radius:12px">Обратите внимание: ' +
        'Перенести eSIM на другой телефон потом нельзя: ставьте её сразу на тот, с которым поедете. Код срабатывает один раз.</p>' +
      ...
      text: (many ? "Ваши eSIM готовы (" + items.length + " шт.): " : "Ваша eSIM готова: ") + (order.label || "") + "\n\n" +
        ...
        "\n\nОбратите внимание: Перенести eSIM на другой телефон потом нельзя: ставьте её сразу на тот, с которым поедете. Код срабатывает один раз." +
        (tipsCN ? "\n\n" + CHINA_TIPS_TEXT : "") +
  ...
  function stepsNoNet(o) {
    const steps = [
      "1. Включите режим полёта на 15 секунд и выключите. Чаще всего помогает именно это: телефон заново регистрируется в сети.",
      "2. Проверьте, что линия eSIM включена, выбрана для мобильного интернета и у неё включён роуминг данных. iPhone: Настройки → Сотовая связь. Android: Настройки → Подключения → Диспетчер SIM-карт и Мобильные сети. Без роуминга данных eSIM за границей не работает.",
      "3. Выберите сеть вручную: в настройках линии eSIM выключите автоматический выбор сети и выберите из списка другого оператора.",
    ...
    try {
      ...
      if (u.suspended) {
        ...
        text = head + "Профиль ещё не установлен на телефон — по данным оператора пакет не активирован.\n" +
          ...
          "Перенести eSIM на другой телефон потом нельзя: ставьте её сразу на тот, с которым поедете.\n" +
    ...
    const body = isData
      ? ("Осталось " + left + " из " + total + " — при активном интернете это меньше дня. " +
         (canTopup ? "Гигабайты добавятся на эту же eSIM, переустанавливать ничего не нужно. " +
                     "Если интернет не появится через пару минут, выключите и включите режим полёта: телефон заново зарегистрируется в сети."
